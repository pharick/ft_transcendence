...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_INTERNAL_API_URL}/completed/${completedGameId}`);
...
const CompletedGamePage: NextPage<CompletedGamePageProps> = ({ completedGameInfo }) => {
  return (
    <h1>Game <b>{completedGameInfo.hostUser.username}</b> vs. <b>{completedGameInfo.guestUser.username}</b> is completed</h1>
  );
};
